...
class SHT7x:
    ...
    def read_byte(self, comm_end = False):
        v = [0] * len(self.data_pin)
        for i in range(8):
            for j, bit in enumerate(self.r_data()):
                v[j] = 2*v[j] + bit
            self.sckPulse()
            self.dly(0.5)
    ...
    def send_byte(self, byte):
        ...
        if not (ack == [0] * len(self.data_pin)):
    ...
    def wait_for_measurement(self):
        ...
        for i in range(320):
            ...
            #if self.r_data() == 0:
            if self.r_data() == [0] * len(self.data_pin):
                return
    ...
    def measure(self, cmd, temp_correction = False):
        ...
        result = [0] * len(self.data_pin)
        if cmd == TEMP:
            for i, r in enumerate(raw):
                result[i] = (0.04 if self.lower_res else 0.01)*r + self.d1

        if cmd == HUM:
            rh_lin = [0] * len(self.data_pin)
            for i, r in enumerate(raw):
